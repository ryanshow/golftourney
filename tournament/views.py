...
def invoice_pdf(request, token):
    """Serve the PDF invoice for a registration, identified by its secret token."""
...
def _build_invoice_pdf(reg):
    """Return the raw PDF bytes for a registration invoice."""
    from .models import TournamentInfo  # avoid circular import at module level
    tournament = TournamentInfo.get_instance()
    ...
    for i in range(max(len(bill_lines), len(from_lines))):
    ...
    pdf.set_text_color(*WHITE)
    pdf.set_font("Helvetica", "B", 10)
    pdf.cell(badge_w, 8, f"  {reg.get_payment_status_display().upper()}", fill=True, new_x="RIGHT", new_y="TOP")
    ...
    if players:
        ...
        pdf.set_text_color(*BLACK)
        for p in players:
            pdf.cell(0, 5, f"  {p.slot}.  {p.name or 'TBD'}", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(4)
    ...
    pdf.set_line_width(0.3)
    notice_y = pdf.get_y()
    pdf.rect(MARGIN, notice_y, usable_w, 16, "FD")
